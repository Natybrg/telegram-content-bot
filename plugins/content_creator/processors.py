...
from services.media.youtube import calculate_timeout
...
from .utils import create_progress_bar, get_progress_stage
from .cleanup import schedule_cleanup, cleanup_session_files
...
async def process_content(client: Client, message: Message, session, status_msg: Message):
    ...
    # ========== מעקב פעולות נוכחיות ==========
    # מעקב אחר כל הפעולות שהמשתמש ציין
    current_operation = ""  # שם הפעולה הנוכחית
    current_operation_percent = 0  # אחוז מהפעולה הנוכחית
    ...
    def get_status_text(include_queue_info=False, queue_status=None):
    ...
    try:
        ...
        await update_status("הורדה של תמונה", 0, 0)
        ...
        # ========== שלב 2: עיבוד MP3 ==========
        await update_status("הורדה של סינגל", 12, 1)
        ...
        output_mp3_path = config.DOWNLOADS_PATH / target_mp3_name
        ...
        if session.need_video:
            await update_status("הורדה של קליפ לטלגרם (מיוטיוב)", 43, 0)
            logger.info(f"📥 Starting YouTube video download in background for user {user_id}")
            logger.info(f"  URL: {session.youtube_url}")
            
            # יוצרים פונקציה עוטפת עם retry logic
            async def download_video_with_retry(max_retries=3):
                """
                פונקציה עוטפת להורדת וידאו עם retry חכם + timeout דינמי
                - 3 ניסיונות
                - Timeout דינמי: כולל הורדה + המרה (המרה כבדה לוקחת יותר זמן)
                - Exponential backoff: 5s → 10s → 20s
                - התקדמות בזמן אמת של המרת FFmpeg
                """
                # נניח 600MB ככלל אגודל (בפועל יהיה timeout ראשוני)
                estimated_size_mb = 600
                
                # חישוב timeout כולל: הורדה + המרה כבדה (AV1/VP9 → H.264)
                # המרה כבדה: 8 דקות לכל 100MB (מינימום 15 דקות) - הוגדל
                download_timeout = calculate_timeout(estimated_size_mb, "download")  # 5 דקות לכל 100MB, מינימום 10 דקות
                # נניח המרה כבדה (AV1/VP9) - זה הגרוע ביותר
                conversion_timeout = calculate_timeout(estimated_size_mb, "conversion", "av1", "opus")
                
                # Timeout כולל = הורדה + המרה + 50% מרווח ביטחון (הוגדל מ-20%)
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        def ffmpeg_progress_callback(percent, current_time, eta):
                            ...
                            nonlocal current_operation, current_operation_percent
                            current_operation = f"עיבוד קליפ טלגרם: {percent}%"
                            current_operation_percent = progress_stage
                        ...
                        if video_result and video_result[0] and os.path.exists(video_result[0]):
                            ...
                            current_operation = "הורדה של קליפ לטלגרם (מיוטיוב)"
                            current_operation_percent = 100
                            return True
                        else:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            await update_status(f"ניסיון {attempt + 1} נכשל (timeout)", 43, 0)
                            await asyncio.sleep(delay)
                    except Exception as e:
                        logger.error(f"❌ [YOUTUBE] שגיאה בניסיון {attempt + 1}: {e}")
                        if attempt < max_retries - 1:
                            delay = 5 * (2 ** attempt)
                            await update_status(f"ניסיון {attempt + 1} נכשל", 43, 1)
                            await asyncio.sleep(delay)
                ...
                await update_status("הורדה של קליפ לטלגרם (מיוטיוב) - נכשל", 43, 0)
                return False
            
            # יוצרים task שרץ ברקע
        ...
        await update_status("העלאת תמונה לטלגרם", 50, 0)
        ...
        if mp3_size_mb > config.TELEGRAM_MAX_FILE_SIZE_MB:
            raise Exception(f"MP3 גדול מדי ל-Telegram: {mp3_size_mb:.2f}MB > {config.TELEGRAM_MAX_FILE_SIZE_MB}MB")
        ...
        upload_status['telegram']['image'] = True
        upload_status['telegram']['audio'] = True
        upload_progress['telegram']['image'] = 100
        upload_progress['telegram']['audio'] = 100
        await update_status("העלאת סינגל לטלגרם", 67, 0)
        
        # ========== העלאה לערוצי טלגרם (תמונה + MP3) ==========
        if config.PUBLISH_TO_CHANNELS:
            try:
                ...
                if telegram_channels:
                    ...
                    logger.info(f"📋 [TELEGRAM → CHANNEL] רשימת ערוצים: {telegram_channels}")
        ...
        if config.WHATSAPP_ENABLED:
            try:
                await update_status("העלאת תמונה לוואטסאפ", 79, 0)
                ...
                if whatsapp_groups:
                    ...
                    executor = config.executor_manager.get_executor()
                    ...
                    try:
                        ...
                        if session.processed_image_path and os.path.exists(session.processed_image_path):
                            ...
                            if image_result.get('success') and image_result.get('sent_to'):
                                ...
                                upload_status['whatsapp']['image'] = True
                                upload_progress['whatsapp']['image'] = 100
                                upload_results['whatsapp']['image'] = {
                                    "success": True,
                                    "size_mb": round(os.path.getsize(session.processed_image_path) / (1024*1024), 1),
                                    "sent_to": len(image_result['sent_to'])
                                }
                                await update_status("העלאת תמונה לוואטסאפ", 80, 0)
                            else:
                                logger.warning(f"⚠️ [WHATSAPP] שליחת תמונה נכשלה: {image_result.get('errors', [])}")
                                errors.append({"platform": "whatsapp", "file_type": "image", "error": str(image_result.get('errors', []))})
                                whatsapp_success = False
                                await update_status("העלאת תמונה לוואטסאפ - נכשל", 80, 0)
                        else:
                        ...
                        if session.processed_mp3_path and os.path.exists(session.processed_mp3_path):
                            ...
                            if mp3_size <= config.WHATSAPP_MAX_FILE_SIZE_BYTES:
                                ...
                                if mp3_result.get('success') and mp3_result.get('sent_to'):
                                    ...
                                    upload_status['whatsapp']['audio'] = True
                                    upload_progress['whatsapp']['audio'] = 100
                                    upload_results['whatsapp']['audio'] = {
                                        "success": True,
                                        "size_mb": round(mp3_size / (1024*1024), 1),
                                        "sent_to": len(mp3_result['sent_to'])
                                    }
                                    await update_status("העלאת סינגל לוואטסאפ", 85, 0)
                                else:
                                    logger.warning(f"⚠️ [WHATSAPP] שליחת MP3 נכשלה: {mp3_result.get('errors', [])}")
                                    errors.append({"platform": "whatsapp", "file_type": "audio", "error": str(mp3_result.get('errors', []))})
                                    whatsapp_success = False
                                    await update_status("העלאת סינגל לוואטסאפ - נכשל", 85, 0)
                            else:
                    ...
                    logger.info("ℹ️ [WHATSAPP] אין קבוצות לשליחה")
        ...
        if video_download_task:
            await update_status("ממתין לסיום הורדת הווידאו", 85, 0)
            ...
            if video_success and session.video_high_path and os.path.exists(session.video_high_path):
                ...
                await update_status("העלאת קליפ לטלגרם", 99, 0)
                ...
                # בדיקת גודל מקסימלי ל-Telegram (2GB)
                if video_size_mb > config.TELEGRAM_MAX_FILE_SIZE_MB:
                    raise Exception(f"וידאו גדול מדי ל-Telegram: {video_size_mb:.2f}MB > {config.TELEGRAM_MAX_FILE_SIZE_MB}MB")
                ...
                upload_status['telegram']['video'] = True
                upload_progress['telegram']['video'] = 100
                await update_status("העלאת קליפ לטלגרם", 100, 0)
                    ...
                    try:
                        ...
                        if not hasattr(session, 'upload_video_path') or not session.upload_video_path:
                            ...
                            if telegram_video_channels:
                                ...
                                # טעינת ערוצים ל-storage לפני שליחה (חשוב מאוד!)
                                logger.info(f"🔄 [TELEGRAM → CHANNEL] טוען ערוצים ל-storage של ה-{client_type}...")
                                for channel_id in telegram_video_channels:
                                    try:
                                        logger.info(f"🔍 [TELEGRAM → CHANNEL] בודק ערוץ: {channel_id}")
                                        chat_obj = await channel_client.get_chat(channel_id)
                                        logger.info(f"✅ [TELEGRAM → CHANNEL] ערוץ {channel_id} נטען בהצלחה: {chat_obj.title if hasattr(chat_obj, 'title') else 'N/A'}")
                                ...
                                video_result = await send_to_telegram_channels(
                                    ...
                                    first_channel_id=telegram_video_channels[0] if telegram_video_channels else None,
                                    protected_channels=[],
                                    **video_kwargs
                                )
                ...
                if config.WHATSAPP_ENABLED:
                    try:
                        await update_status("עיבוד קליפ וואטסאפ", 80, 0)
                        ...
                        if whatsapp_video_groups:
                            ...
                            try:
                                ...
                                if video_result.get('success') and video_result.get('sent_to'):
                                    ...
                                    # עדכון מעקב התקדמות
                                    upload_status['whatsapp']['video'] = True
                                    upload_progress['whatsapp']['video'] = 100
                                    upload_results['whatsapp']['video'] = {
                                        "success": True,
                                        "size_mb": round(video_size / (1024*1024), 1),
                                        "sent_to": len(video_result['sent_to'])
                                    }
                                    await update_status("העלאת קליפ לוואטסאפ", 99, 0)
                                else:
                                    logger.warning(f"⚠️ [WHATSAPP] שליחת וידאו נכשלה: {video_result.get('errors', [])}")
                                    errors.append({"platform": "whatsapp", "file_type": "video", "error": str(video_result.get('errors', []))})
                                    await update_status("העלאת קליפ לוואטסאפ - נכשל", 99, 0)
                            finally:
                                if 'whatsapp' in locals():
                                    whatsapp.close()
                        else:
                ...
                errors.append({"platform": "telegram", "file_type": "video", "error": "הורדת וידאו נכשלה לאחר 3 ניסיונות"})
                await update_status("הורדה של קליפ לטלגרם (מיוטיוב) - נכשל", 100, 0)
        
        # ========== סיום ==========
        # קביעת הצלחה או כישלון על בסיס העלאות לערוצים ווואטסאפ
        channel_image_success = upload_status['telegram']['image']
        channel_audio_success = upload_status['telegram']['audio']
        channel_video_success = not session.need_video or upload_status['telegram']['video']
        
        all_success = (
            channel_image_success and 
            channel_audio_success and
            channel_video_success and
            (not config.WHATSAPP_ENABLED or whatsapp_success)
        )
        ...
        if config.WHATSAPP_ENABLED and not whatsapp_success:
        ...
        errors.append({"platform": "general", "file_type": "processing", "error": str(e)})
        try:
            error_text = (
                f"❌ **שגיאה בעיבוד!**\n\n"
                f"פרטי שגיאה: {str(e)}\n\n"
...
async def process_instagram_upload(client: Client, message: Message, session, status_msg: Message):
    ...
    try:
        ...
        # עדכון סטטוס
        if media_type == "video":
            await update_status("הורדה של קליפ מאינסטגרם", 0, 0)
        else:
            await update_status("הורדה של תמונה מאינסטגרם", 0, 0)
        ...
        if telegram_channels:
            ...
            if media_type == "video":
                await update_status("העלאת קליפ לטלגרם", 50, 0)
            else:
                await update_status("העלאת תמונה לטלגרם", 50, 0)
            ...
            try:
                telegram_result = await send_to_telegram_channels(
                    ...
                    channels=telegram_channels,
                    first_channel_id=telegram_channels[0] if telegram_channels else None,
                    protected_channels=[]
                )
                ...
                if telegram_result.get('success'):
                    ...
                    await update_status("העלאת קליפ לטלגרם" if media_type == "video" else "העלאת תמונה לטלגרם", 67, 0)
                else:
                    error_msg = telegram_result.get('error', 'Unknown error')
                    errors.append({"platform": "telegram", "file_type": media_type, "error": error_msg})
                    logger.error(f"❌ [TELEGRAM] שגיאה: {error_msg}")
                    await update_status("העלאת קליפ לטלגרם - נכשל" if media_type == "video" else "העלאת תמונה לטלגרם - נכשל", 67, 0)
            except Exception as e:
                errors.append({"platform": "telegram", "file_type": media_type, "error": str(e)})
        ...
        if config.WHATSAPP_ENABLED:
            ...
            if whatsapp_groups:
                ...
                if media_type == "video":
                    await update_status("העלאת קליפ לוואטסאפ", 79, 0)
                else:
                    await update_status("העלאת תמונה לוואטסאפ", 79, 0)
                ...
                try:
                    ...
                    if whatsapp is None:
                        ...
                        if whatsapp_result.get('success'):
                            ...
                            await update_status("העלאת קליפ לוואטסאפ" if media_type == "video" else "העלאת תמונה לוואטסאפ", 85, 0)
                        else:
                            error_msgs = whatsapp_result.get('errors', [])
                            for error_msg in error_msgs:
                                errors.append({"platform": "whatsapp", "file_type": media_type, "error": error_msg})
                            logger.error(f"❌ [WHATSAPP] שגיאות: {error_msgs}")
                            await update_status("העלאת קליפ לוואטסאפ - נכשל" if media_type == "video" else "העלאת תמונה לוואטסאפ - נכשל", 85, 0)
                except Exception as e:
                    errors.append({"platform": "whatsapp", "file_type": media_type, "error": str(e)})
                    logger.error(f"❌ [WHATSAPP] שגיאה בהעלאה: {e}", exc_info=True)
            else:
                logger.info("ℹ️ [WHATSAPP] אין קבוצות להעלאה")
        else:
...
async def process_video_only(client: Client, message: Message, session, status_msg: Message):
    ...
    try:
        ...
        await update_status("הורדה של קליפ לטלגרם (מיוטיוב)", 0, 0)
        logger.info(f"📥 Starting YouTube video download for user {user_id}")
        logger.info(f"  URL: {session.youtube_url}")
        
        # פונקציה עוטפת עם retry logic
        async def download_video_with_retry(max_retries=3):
            """פונקציה עוטפת להורדת וידאו עם retry חכם + timeout דינמי"""
            estimated_size_mb = 600
            ...
            logger.info(f"⏱️ [YOUTUBE] Timeout כולל: {dynamic_timeout}s ({dynamic_timeout//60} דקות)")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if video_result and video_result[0] and os.path.exists(video_result[0]):
                        ...
                        raise Exception("הורדה נכשלה - לא הוחזר וידאו")
        ...
        await update_status("עיבוד קליפ טלגרם", 50, 0)
        logger.info("✅ [YOUTUBE] הורדת וידאו הושלמה, מתחיל העלאה!")
        ...
        # ========== שלב 3: העלאה לטלגרם ==========
        await update_status("העלאת קליפ לטלגרם", 67, 0)
            ...
            try:
                ...
                if telegram_video_channels:
                    ...
                    if video_result['success']:
                        ...
                        upload_status['telegram']['video'] = True
                        upload_progress['telegram']['video'] = 100
                        await update_status("העלאת קליפ לטלגרם", 79, 0)
                    else:
                        error_msg = video_result.get('error', 'Unknown error')
                        logger.error(f"❌ [TELEGRAM → CHANNEL] שגיאה בשליחת וידאו: {error_msg}")
                        errors.append({"platform": "telegram", "file_type": "video", "error": str(error_msg)})
                ...
                logger.error(f"❌ [TELEGRAM → CHANNEL] שגיאה בפרסום וידאו לערוצים: {e}", exc_info=True)
                errors.append({"platform": "telegram", "file_type": "video", "error": str(e)})
                # אם השליחה לטלגרם נכשלה, נוודא שהקובץ עדיין קיים לפני שליחה ל-WhatsApp
                if session.upload_video_path and not os.path.exists(session.upload_video_path):
                    logger.warning(f"⚠️ [TELEGRAM → WHATSAPP] קובץ upload_video_path נמחק או לא קיים: {session.upload_video_path}")
                    # נסיר את upload_video_path מהרשימה כדי שהקוד ינסה להשתמש ב-video_medium_path או video_high_path
        ...
        if config.WHATSAPP_ENABLED:
            try:
                ...
                if whatsapp_video_groups:
                    ...
                    try:
                        ...
                        if video_result.get('success') and video_result.get('sent_to'):
                            ...
                            upload_status['whatsapp']['video'] = True
                            upload_progress['whatsapp']['video'] = 100
                            await update_status("וידאו נשלח לוואטסאפ", 100, 1)
                        else:
                            logger.warning(f"⚠️ [WHATSAPP] שליחת וידאו נכשלה: {video_result.get('errors', [])}")
                            errors.append({"platform": "whatsapp", "file_type": "video", "error": str(video_result.get('errors', []))})
                            await update_status("שליחת וידאו נכשלה", 100, 0)
                    finally:
                ...
                logger.error(f"❌ [WHATSAPP] שגיאה בשליחת וידאו: {e}", exc_info=True)
                errors.append({"platform": "whatsapp", "file_type": "video", "error": str(e)})
        
        # ========== סיום ==========
        all_success = (
            upload_status['telegram']['video'] and
            (not config.WHATSAPP_ENABLED or upload_status['whatsapp']['video'])
        )
